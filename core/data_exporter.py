    ...
    try:
        ...
        lista_de_tuplas = [
            (
                ...
                d.get('tensao_estimada_mv', 0.0) if d.get('tensao_estimada_mv') is not None else np.nan,
                d.get('erro_obs_mv', 0.0) if d.get('erro_obs_mv') is not None else np.nan
            )
            for d in data
        ]
