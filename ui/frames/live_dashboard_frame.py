...
class LiveDashboardFrame(ctk.CTkFrame):
    ...
    def __init__(self, master: Any, controller: Any):
        ...
        ctk.CTkButton(self.sidebar_frame, text="Controle e Tesão", command=lambda: self.select_graph('controle_tensao')).pack(pady=10, padx=20)
        ...
        self.plotter = GraphManager(self.fig, self.ax, max_points=200)
        ...
        ctk.CTkLabel(self.bottom_bar, text="Setpoint (V):").pack(side="left", padx=10)
    ...
    def _update_stats_bar(self) -> None:
        ...
        try:
            ...
            if 'last_y1' in stats:
                ...
                self.label_last_y.configure(text=f"Erro Atual: {stats.get('last_y', '--')} mV")
                self.label_avg_y.configure(text=f"Média do Erro: {stats.get('avg_y', '--')}")
            else:
